...
class HTTPMethodScanner(BaseScanner):
    ...
    def _check_options(self):
        ...
        if allow:
            ...
            dangerous = [m for m in ["PUT", "DELETE", "PATCH", "CONNECT", "TRACE"]
                         if m in allow.upper()]
            if dangerous:
                log("WARN", f"Allow 头含危险方法: {', '.join(dangerous)}")
    ...
    def _check_put(self):
        ...
        if r and r.status_code in [200, 201, 204]:
            ...
            log("OK", f"PUT 方法: {r.status_code}（不可用）")

    def _check_delete(self):
        # 只测试明显不存在的路径，避免删真实资源
        test_url = self.url("__nonexistent_webscanner_file_xyz__.txt")
        r = self.request("DELETE", test_url)
        if r and r.status_code in [200, 204]:
            log("WARN", "[DELETE] HTTP DELETE 方法可能已启用")
            self.result.add("HTTP 方法", "MEDIUM",
