...
**v11 亮点**：62 个检测模块、内置 **1500+ 检测规则/签名**；在已很完整的注入面之外，新增 HTTP 参数污染、动词篡改/WebDAV、PHP 包装器/LFI 向量、DOM XSS 静态分析、CORS 高级绕过、会话固定、WebSocket 检测、全站 PII/密钥深扫、安全头策略深度、敏感页缓存等模块；软 404 改用相似度识别、新增 `--max-requests` 请求预算等优化。
