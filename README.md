...
**v10 亮点**：52 个检测模块、内置 **1300+ 检测规则/签名**，注入类漏洞覆盖非常完整（SQL/NoSQL/LDAP/XPath/XXE/SSTI/SSI/EL-OGNL/命令注入/LFI/路径穿越/CRLF/邮件头/CSV/原型链污染）；并新增反序列化指示、源码泄露、调试端点、CSP/Cookie 深度分析、GraphQL/JWT 深度、OAuth 配置、缓存欺骗等模块；YAML 模板引擎可无代码扩展规则。

---

## 检测模块（52 个）

**信息收集与指纹**：爬虫、信息收集、指纹识别（90+ 规则）、版本漏洞提示、robots/sitemap/well-known 情报、子域名枚举、子域名接管指纹。

**配置与传输**：安全头、SSL/TLS、CSP 深度评估、Cookie 深度分析、配置错误/点击劫持、前端安全（SRI/混合内容）、HTTP 方法、Host 头注入、CORS、CSRF、OAuth/OIDC 配置。

**信息泄露**：敏感信息、JS 密钥/端点、敏感路径暴露（600+ 路径）、源码泄露、调试端点、反序列化指示、模板签名库（YAML，可扩展）、API 文档发现、CMS 专项。

**注入与漏洞（覆盖完整）**：SQL 注入、**NoSQL 注入**、**LDAP 注入**、**XPath 注入**、XSS/SSTI、**SSI 注入**、**EL/OGNL 表达式注入**、LFI/命令注入、路径穿越、XXE、SSRF、CRLF 注入、**邮件头注入**、**CSV 公式注入**、**原型链污染**、开放重定向、缓存投毒、**缓存欺骗**、Log4Shell、JWT 安全（含 kid/jku/算法混淆分析）、GraphQL（含深度检测）。

**主动扫描**：端口扫描（68 高危端口）、目录枚举。

> 启动时会显示实际加载的**内置检测规则/签名总量**（模板 + 敏感路径 + 指纹 +
> 载荷 + 端口 + 字典），当前 **1000+**，且可通过模板持续扩展。
